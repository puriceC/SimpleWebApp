...
public class DirectorServiceImpl implements DirectorService {
	...
	@Override
	public List<Director> getPage(int pageSize, int pageNumber, String sortBy) {
		...
		if (Arrays.stream(Director.class.getDeclaredFields()).noneMatch(field -> field.getName().equals(sortBy))) {
		...
		Page<Director> directorPage = repository.findAll(pageable);
	...
	public Director insert(Director director) {
		logger.debug(format("insert called with %s", director));
		return repository.save(director);
	}

	@Override
	public void update(Integer id, Director director) {
		logger.debug(format("update called with id = %d and %s", id, director));
		director.setId(id);
	...
	public void delete(Integer id) {
		...
		Director toBeDeleted = repository.getById(id);
